...
const ProblemReportItem: FC<IProps> = ({ problem, handleProblemClick }) => {
  ...
  return (
    <ListItemButton onClick={handleProblemClick}>
      ...
      <ListItemText color="primary" primary={convertMillisecondsToDate(problem.created_date)} />
      <ListItemText color="primary" secondary={problem.title} />
      <Chip label={getPriorityTitleById(problem.priority_id)} color={priorityColor} sx={{ width: "100px" }} />
    </ListItemButton>
  );
};
